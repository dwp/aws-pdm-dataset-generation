locals {
  pdm_object_tagger_image = "${local.account.management}.${module.vpc.ecr_dkr_domain_name}/dataworks-s3-object-tagger${var.pdm_object_tagger_version}"
}
...
data "aws_iam_policy_document" "pdm_object_tagger" {
  statement {
    ...
    resources = [
      data.terraform_remote_state.adg.outputs.published_bucket.id,
    ]
  }
}
...
resource "aws_batch_job_definition" "pdm_object_tagger" {
  ...
  {
      "command": ["-m", "Ref::manifest"],
      "image": "${var.pdm_object_tagger_image}",
      ...
      "environment": [
          ...
          {"name": "S3_PREFIX", "value": "pdm-dataset/"},
          {"name": "CSV_LOCATION", "value": "NOT_SET"}
      ],
      "ulimits": [
        {
          "hardLimit": 1024,
