...
resource "aws_cloudwatch_event_target" "pdm_success_start_object_tagger" {
  ...
  arn       = data.terraform_remote_state.aws_s3_object_tagger.outputs.s3_object_tagger_batch_job_def.pdm_job_queue.arn
  role_arn  = aws_iam_role.allow_batch_job_submission.arn

  batch_target {
    job_definition = data.terraform_remote_state.aws_s3_object_tagger.outputs.s3_object_tagger_batch_job_def.job_definition.id
