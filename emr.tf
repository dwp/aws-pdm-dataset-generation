resource "aws_emr_cluster" "cluster" {
  ...
  service_role         = aws_iam_role.pdm_dataset_generator.arn
  ...
  ec2_attributes {
    ...
    additional_master_security_groups = aws_security_group.pdm_dataset_generation.id
    additional_slave_security_groups  = aws_security_group.pdm_dataset_generation.id
  ...
  step {
    ...
    hadoop_jar_step {
      ...
      args = [
        "spark-submit",
        format("s3://%s/%s", data.terraform_remote_state.common.outputs.config_bucket.id, aws_s3_bucket_object.generate-pdm-dataset-script.key),
        "--deploy-mode",
        "cluster",
        "--master",
        "yarn",
  ...
  depends_on = [
    aws_s3_bucket_object.generate-pdm-dataset-script
  ]
