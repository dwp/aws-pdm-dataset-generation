locals {
  ...
  no_proxy             = "169.254.169.254,data.${data.terraform_remote_state.metrics_infrastructure.outputs.pdm_pushgateway_hostname},${join(",", formatlist("%s.%s", local.endpoint_services, local.amazon_region_domain))}"
