...
#        TODO Lock down pdm dataset EMR instance profile DW-3618
...
resource "aws_security_group_rule" "ingress_https_vpc_endpoints_from_emr" {
  ...
  to_port                  = 443
  type                     = "ingress"
  source_security_group_id = aws_security_group.pdm_dataset_generation.id
}
...
output "pdm_dataset_generation_sg" {
  value = aws_security_group.pdm_dataset_generation
}
