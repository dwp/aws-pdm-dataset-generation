...
def spark_process(collection):
    ...
    raw_df = df.select(df.data, F.get_json_object(df.data, "$.message.encryption.encryptedEncryptionKey").alias("encryptedKey"),
                       F.get_json_object(df.data, "$.message.encryption.keyEncryptionKeyId").alias(
        "keyEncryptionKeyId"),
        F.get_json_object(
        ...
        F.get_json_object(
        df.data, "$.message.db").alias("db_name"),
        F.get_json_object(df.data, "$.message.collection").alias(
        "collection_name"),
        F.get_json_object(df.data, "$.message._id").alias("id"))
    key_df = raw_df.withColumn("key", get_plain_key(
        raw_df["encryptedKey"], raw_df["keyEncryptionKeyId"]))
    decrypted_df = key_df.withColumn("decrypted_db_object", decryption(
        key_df["key"], key_df["iv"], key_df["data"]))
    validated_df = decrypted_df.withColumn(
        "validated_db_object", validation(decrypted_df["decrypted_db_object"]))
    sanitised_df = validated_df.withColumn("sanitised_db_object", sanitise(
        validated_df["validated_db_object"], validated_df["db_name"], validated_df["collection_name"]))
...
def tag_objects(prefix, tag_value):
    ...
    for key in client_s3.list_objects(Bucket=s3_publish_bucket, Prefix=prefix)[
        ...
        client_s3.put_object_tagging(
            ...
            Key=key["Key"],
            Tagging={"TagSet": [
                {"Key": "collection_tag", "Value": tag_value}]},
        )
...
def get_spark_session():
    spark = (
        ...
        .appName("aws-pdm-dataset-generator")
        .enableHiveSupport()
        .getOrCreate()
    )
...
def wrap_dates(db_object):
    last_modified_date_time_as_string = retrieve_last_modified_date_time(
        db_object)
...
def retrieve_last_modified_date_time(db_object):
    ...
    created_date_time = retrieve_date_time_element(
        "createdDateTime", db_object)
...
def format_date_to_valid_outgoing_format(current_date_time):
    ...
    parsed_date_time = datetime.datetime.strftime(
        parsed_date_time, "%Y-%m-%dT%H:%M:%S.%f")
...
def get_valid_parsed_date_time(time_stamp_as_string):
    ...
    for time_stamp_fmt in valid_timestamps:
        try:
            ...
            print(
                f"timestampAsString did not match valid format {time_stamp_fmt}")
