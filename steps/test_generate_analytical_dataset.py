import generate_pdm_dataset
from generate_pdm_dataset import sanitize, validate, retrieve_id, retrieve_last_modified_date_time, retrieve_date_time_element, replace_element_value_wit_key_value_pair, wrap_dates, get_valid_parsed_date_time, format_date_to_valid_outgoing_format
...
def test_retrieve_value_string_when_date_element_is_string():
    ...
    actual = retrieve_date_time_element(
        'dateTimeTestElement', decrypted_db_object)
    assert expected == actual
...
def test_main(monkeypatch):
    generate_pdm_dataset.get_plaintext_key_calling_dks = mock_get_plaintext_key_calling_dks
    monkeypatch.setattr(generate_pdm_dataset,
                        'get_published_db_name', mock_get_published_db_name)
    monkeypatch.setattr(generate_pdm_dataset,
                        'get_staging_db_name', mock_get_staging_db_name)
    monkeypatch.setattr(generate_pdm_dataset,
                        'get_tables', mock_get_tables)
    monkeypatch.setattr(generate_pdm_dataset,
                        'get_spark_session', mock_get_spark_session)
    monkeypatch.setattr(generate_pdm_dataset,
                        'get_dataframe_from_staging', mock_get_dataframe_from_staging)
    monkeypatch.setattr(generate_pdm_dataset,
                        'get_plaintext_key_calling_dks', mock_get_plaintext_key_calling_dks)
    monkeypatch.setattr(generate_pdm_dataset,
                        'persist_parquet', mock_persist_parquet)
    monkeypatch.setattr(generate_pdm_dataset,
                        'create_hive_on_published', mock_create_hive_on_published)
    monkeypatch.setattr(generate_pdm_dataset,
                        'retrieve_secrets', mock_retrieve_secrets)
    monkeypatch.setattr(generate_pdm_dataset,
                        'get_collections', mock_get_collections)
    monkeypatch.setattr(generate_pdm_dataset,
                        'tag_objects', mock_tag_objects)
    generate_pdm_dataset.main()
...
def mock_get_spark_session():
    spark = (
        ...
        .appName("aws-pdm-dataset-generator-test")
        .enableHiveSupport()
        .getOrCreate()
    )
    return spark
